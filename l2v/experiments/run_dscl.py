...
from l2v.utils.collates import eval_collate_fn, sup_not_scl_train_collate_fn
from l2v.models.dscl import SimCSEModel
from l2v.utils.l2v import setup_l2v
import argparse
import wandb

wandb.init(project="simcse-l2v", name="dscl")
...
wandb.init(project="simcse-l2v", name=f"dscl-{args.dataset_name}")
...
dscl_model = SimCSEModel(model=model)
...
# パラメータを連続にする
for param in dscl_model.parameters():
    param.data = param.data.contiguous()
print(type(dscl_model).__name__)
...
dscl_model.decoder.save_pretrained(decoder_path)
